...
public class TBTobacco extends Item implements ITobacco {
    ...
    public void performTobaccoEffect(EntityPlayer smoker, int metadata, boolean isSilverwood) {
        switch (metadata) {
            case 0: {
                if (isSilverwood && smoker.worldObj.rand.nextFloat() <= 0.3F) TBUtils.addWarpToPlayer(smoker, -1, 0);

                if (!isSilverwood && smoker.worldObj.rand.nextFloat() <= 0.1F) TBUtils.addWarpToPlayer(smoker, -1, 0);
            ...
            case 1: {
                if (!smoker.worldObj.isRemote) {
                    ...
                    if (!isSilverwood) TBUtils.addWarpToPlayer(smoker, 3, 0);
                }
            ...
            case 4: {
                if (!smoker.worldObj.isRemote) {
                    ...
                    for (int i = 0; i < (isSilverwood ? 20 : 10); ++i) {
                        ...
                        if (a == Aspect.TAINT && !isSilverwood) TBUtils.addWarpToPlayer(smoker, 1, 0);
                    }
                }
                break;
            }
            ...
            case 7: {
                if (!smoker.worldObj.isRemote) {
                    if (!isSilverwood) {
                        ...
                        if (smoker.worldObj.rand.nextFloat() <= 0.4F) TBUtils.addWarpToPlayer(smoker, 1, 1);
                    } else {
            ...
            case 8: {
                ...
                if (isSilverwood) aspects.remove(Aspect.TAINT);
