...
public class ItemSmokingPipe extends Item {
    ...
    public ItemStack getTobacco(EntityPlayer p) {
        for (int i = 0; i < p.inventory.getSizeInventory(); ++i) {
            ...
            if (stk != null && stk.getItem() != null && stk.getItem() instanceof ITobacco) return stk;
        }
        return null;
    }

    public ItemStack onItemRightClick(ItemStack stack, World w, EntityPlayer player) {
        if (this.getTobacco(player) != null) player.setItemInUse(stack, getMaxItemUseDuration(stack));
        return stack;
    }
