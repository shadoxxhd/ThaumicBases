...
public class BlockEntityDeconstructor extends BlockContainer {
    ...
    public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer p, int side, float vecX, float vecY,
        ...
        if (p.getCurrentEquippedItem() != null) {
            if (p.getCurrentEquippedItem()
                .getItem() instanceof ItemBlockCrystal) {
                if (ResearchManager.isResearchComplete(p.getCommandSenderName(), "TB.EntityDecAdv"))
                    if (p.getCurrentEquippedItem()
                        .getItemDamage() < 6) {
                            ...
                            switch (p.getCurrentEquippedItem()
                                .getItemDamage()) {
                                case 0: {
                                    if (!d.hasAir) {
                                        d.hasAir = true;
        ...
        if (p.worldObj.isRemote) p.addChatMessage(
            new ChatComponentText(StatCollector.translateToLocal("tb.txt.linkEntDecon")).setChatStyle(
