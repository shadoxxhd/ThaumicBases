...
public class BlockSpike extends Block {
    ...
    public void onEntityCollidedWithBlock(World w, int x, int y, int z, Entity collider) {
        ...
        if (meta == 2 || meta == 3) {
            if (w.isBlockIndirectlyGettingPowered(x, y, z)) return;

            if (!(collider instanceof EntityItem)) collider.attackEntityFrom(DamageSource.cactus, 14);
        ...
        if (meta == 4 || meta == 5) {
            ...
            if (collider instanceof EntityPlayer) {
                ...
                if (boots != null && boots.getItem() instanceof ItemVoidArmor) return;
            }
    ...
    public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer player, int side, float vecX, float vecY,
        ...
        if (cI.getItem() != ConfigItems.itemResource) return false;

        if (cI.getItemDamage() != 7) return false;
