...
public class BlockTBLeaves extends BlockOldLeaf {
    ...
    public boolean isFlammable(IBlockAccess world, int x, int y, int z, ForgeDirection face) {
        if (world.getBlockMetadata(x, y, z) % 8 == 2) return true;

        return super.isFlammable(world, x, y, z, face);
    }

    public int getFlammability(IBlockAccess world, int x, int y, int z, ForgeDirection face) {
        if (world.getBlockMetadata(x, y, z) % 8 == 2) return 0;
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        if (world.getBlockMetadata(x, y, z) % 8 != 3) return super.getLightValue(world, x, y, z);
        else return 11;
    }
