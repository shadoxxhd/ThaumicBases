...
public class TileCampfire extends TileEntity {
    ...
    public void updateEntity() {
        ...
        if (burnTime > 0) {
            ...
            if (logTime > 0) --logTime;
        }

        if (!this.worldObj.isRemote) {
            if (burnTime <= 0) {
                int metadata = this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord);
                if (metadata == 2) this.worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, 1, 3);

                return;
            }
            if (logTime <= 0) {
                int metadata = this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord);
                if (metadata > 0) this.worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, 0, 3);
        ...
        if (!this.worldObj.isRemote) {
            ...
            for (EntityCreature c : creatures) {
                ...
                if (campfireAviodable == null) campfireAviodable = new EntityAIAvoidCampfire(c);
                ...
                if (!hasTask) c.tasks.addTask(1, campfireAviodable);
            }
        }
    }
}
