...
public class TileRelocator extends TileEntity implements IWandable {
    ...
    public void updateEntity() {
        ForgeDirection orientation =
                ForgeDirection.values()[this.worldObj.getBlockMetadata(xCoord, yCoord, zCoord) % 6];
        ...
        if (firstTick || checkCount % 100 == 0) {
            ...
            while (worldObj.getBlock(
                            ...
                            zCoord + opposite.offsetZ * cont)
                    instanceof BlockRelocator) {
                ++cont;
            }
            ...
            if (!this.worldObj
                ...
                if (meta == 0)
                    aabb = AxisAlignedBB.getBoundingBox(
                            xCoord, yCoord - power, zCoord, xCoord + 1, yCoord + 1, zCoord + 1);
                if (meta == 1)
                    aabb = AxisAlignedBB.getBoundingBox(
                            xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1 + power, zCoord + 1);
                if (meta == 2)
                    aabb = AxisAlignedBB.getBoundingBox(
                            xCoord, yCoord, zCoord - power, xCoord + 1, yCoord + 1, zCoord + 1);
                if (meta == 3)
                    aabb = AxisAlignedBB.getBoundingBox(
                            xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1, zCoord + 1 + power);
                if (meta == 4)
                    aabb = AxisAlignedBB.getBoundingBox(
                            xCoord - power, yCoord, zCoord, xCoord + 1, yCoord + 1, zCoord + 1);
                if (meta == 5)
                    aabb = AxisAlignedBB.getBoundingBox(
                            xCoord, yCoord, zCoord, xCoord + 1 + power, yCoord + 1, zCoord + 1);
    ...
    public int onWandRightClick(
        ...
        if (!paramEntityPlayer.isSneaking()) {
            ...
            int meta = paramWorld.getBlockMetadata(xCoord, yCoord, zCoord) > 5 ? 6 : 0;
            paramWorld.setBlockMetadataWithNotify(
                    xCoord, yCoord, zCoord, meta + ForgeDirection.OPPOSITES[paramInt4], 3);
        }
        return 0;
    }
