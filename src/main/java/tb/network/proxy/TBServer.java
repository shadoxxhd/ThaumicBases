...
public class TBServer implements IGuiHandler {
    ...
    public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        ...
        if (ID == 0x421922) {
            ...
            if (ID == 0x421921) return new ContainerThaumicAnvil(player.inventory, world, x, y, z, player);

            if (ID == 0x421920) return new ContainerVoidAnvil(player.inventory, world, x, y, z, player);

            if (ID == 0x421919) return new ContainerRevolver(player.inventory, world, x, y, z);
        }
