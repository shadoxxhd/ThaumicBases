...
public class TBEnchant {
    ...
    public static void setup() {
        elderKnowledge = new EnchantmentElderKnowledge(TBConfig.elderKnowledgeID, 12, EnumEnchantmentType.weapon)
        ...
        if (Loader.isModLoaded("ThaumicTinkerer") && TBConfig.enableTTCompathability) {
            try {
                ...
                reg.invoke(
                        null,
                        tainted,
                        "thaumicbases:textures/enchantments/tainted.png",
                        false,
                        new AspectList().add(Aspect.FIRE, 12).add(Aspect.ENTROPY, 16));
                reg.invoke(
                        null,
                        vaporising,
    ...
    public static Enchantment elderKnowledge, eldritchBane, magicTouch, tainted, vaporising;
}
