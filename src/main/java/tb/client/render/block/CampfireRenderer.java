...
public class CampfireRenderer extends BlockRenderer implements ISimpleBlockRenderingHandler {
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
         ...
         * Random(x+y+z); for(int i = 0; i < 6+rand.nextInt(5); ++i) { double dx = rand.nextDouble(); double dy = 0;
         * double dz = rand.nextDouble(); renderer.setRenderBounds(dx, dy, dz, dx+0.1D, dy+0.1D, dz+0.1D);
        ...
        double yIndex = 0;
        if (world.getBlockMetadata(x, y, z) > 0) for (int i = 0; i < 2; ++i) {
